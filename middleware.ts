...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // NOTE: "/admin/settings" is intentionally NOT owner-only here. Admins
    // are meant to have read-only access to that page (see the fieldset in
    // app/admin/settings/page.tsx and the requireAdmin() check on
    // GET /api/admin/settings) — the page-level "Système" tab and the
    // underlying /api/admin/system route still enforce requireOwner() on
    // their own, so gating the whole page here as owner-only was blocking
    // admins from a page they were explicitly allowed to view.
