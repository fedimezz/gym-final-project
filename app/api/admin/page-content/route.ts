...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = await prisma.pageContent.findMany({ where: { pageKey: { in: PAGE_CONTENT_SCHEMA.map((p) => p.pageKey) } } });
    const savedByPage = new Map(rows.map((r) => [r.pageKey, r.content as Record<string, string>]));
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const row = await prisma.pageContent.upsert({ where: { pageKey }, create: { pageKey, content: clean, updatedBy: auth.user.id }, update: { content: clean, updatedBy: auth.user.id } });
